...
Plans live **inside the repo they change**, under `.claude/plans/`, gitignored.
One repo, one tree. Cross-repo work is two plans that name each other.

```
.claude/plans/
  README.md      # the index: one line per plan, newest first
...
Worktrees stay where they already are: `.claude/worktrees/<slug>/`, also gitignored.
A plan in `open/` owns exactly one worktree with the same slug.

## Filename

`<scope>-<slug>.md`, where `<scope>` is the conventional-commit scope you would put on
the commit — a crate, component, or surface name. No scope for a repo-wide change; then
just `<slug>.md`.

Examples: `auth-token-refresh.md`, `ci-flaky-retry.md`, `api-rate-limit.md`.
...
Never write a check from memory. In order:

1. The repo's `AGENTS.md` or `CLAUDE.md` — if it names a gate, that is the gate.
2. The repo's `Makefile` or `justfile` — a `check`, `verify`, or `lint` target.
3. CI: `.github/workflows/*.yml` — copy the commands the jobs actually run.
4. Only if none of those exist, the toolchain default. For Rust:
   `cargo +nightly fmt --all --check`,
   `cargo clippy --workspace --all-targets --all-features --locked -- -D warnings`,
   `cargo test --workspace`.

A suite that needs Docker, a cluster, or cloud credentials runs only when the plan
names it in `## Checks`. Do not run it because it exists.
